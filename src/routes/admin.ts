...
router.post('/register', (req: Request, res: Response) => {
    if (process.env.ALLOWREGISTER === '1') {
        try {
            ...
            const request = JSON.parse(decrypt((req.body as {data: string}).data)) as registerRequest;
            ...
            }), request.password, ((err: Error, user: UserDocument) => {
                if (err) {
                    switch (err.name) {
                    ...
                    const html = Mail.generateVerifyEmail(request.email, hash, res.__('ts.register.clickheretoverifyemail'), user.username)
                    void SendGrid.send(request.email, res.__('ts.register.clickheretoverifyemailtitle'), html).then(() => {
...
router.post('/help/resetpassword', (req, res) => {
    const decrypted = decrypt((req.body as dataBody).data);
