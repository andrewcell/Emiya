...
router.get(['/category', '/category/:id'], (req: Request, res: Response) => {
    if (typeof req.params.id !== 'undefined' && req.params.id === 'welcome') {
        ...
        mibo.forEach((ambo) => {
            const cardNumber: number = Number(ambo.number);
            if (cardNumber !== -1) {
...
router.get('/data/:key', ((req: Request, res: Response) => {
    ...
    if (key === null || key === undefined || !downloadKey.has(key)) {
...
const ranking = (name: string, ip: string, key: string, sp: any, req: Request) => {
    ...
    if (!existsSync(path.join(__dirname, '../data/', filename))) {
        writeFileSync(path.join(__dirname, '../data/', filename), '{}');
    }
    const rankingData = JSON.parse(readFileSync(path.join(__dirname, '../data/', filename)).toString());
    const arr = name.split('-');
    const cardNumber = arr[0].toString().trim()
    if (!(req!.session!.number >= 1 || ipSession!.get(ip)! >= 5) || ipSession.get(ip) === null) {
        if (rankingData[name] === null || rankingData[name] === undefined) {
            rankingData[name] = 1;
        } else {
            rankingData[name] =  rankingData[name] + 1;
        }
        // (name in rankingData) ? rankingData[name]++ : rankingData[name] = 1;
        if (ipSession.get(ip) !== undefined) {
    ...
    writeFileSync(path.join(__dirname, '../data/', filename), JSON.stringify(rankingData, null, 4));
}
