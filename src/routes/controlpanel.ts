  ...
  const username = req.params.username;
   void User.findOne({username}).then((user) => {
     if (user != null) {
       ...
       void SendGrid.send(user.email, '[DodoSeki] 서버 오류로 재전송된 가입 인증 메일입니다.', html)
